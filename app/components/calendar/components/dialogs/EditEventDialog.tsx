...
export function EditEventDialog({ children, event }: IProps) {
  ...
  const { updateEventReminders } = useReminderNotifications();
  ...
  // Initialize recurrence and reminders from existing event
  useEffect(() => {
  ...
  }, [event.recurrence?.length, event.recurringEventId, event.startDate, event.reminders, recurrenceInitialStart]);
  ...
  const onSubmit = async (values: TEventFormData) => {
    try {
      ...
      const updated = await updateGCalEvent(gId, {
        ...
        reminders: reminders.useDefault ? null : {
          ...reminders,
      ...
      const updatedLocalEvent = { ...convertGCalToLocal(updated), id: event.id, gEventId: gId };
      setLocalEvents(prev => prev.map(e =>
        e.id === event.id ? updatedLocalEvent : e
      ));

      // Update reminders for the edited event
      updateEventReminders(updatedLocalEvent);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onToggle}>
      ...
      <DialogContent 
        ...
        className={cn(
          "border-gray-700 max-h-[90vh] flex flex-col",
