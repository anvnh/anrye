...
export const useWikilinkAutocomplete = ({
  ...
  const [autocompleteState, setAutocompleteState] =
    useState<WikilinkAutocompleteState>({
      isOpen: false,
      suggestions: [],
      query: "",
      position: null,
  ...
  const checkWikilinkContext = useCallback(
  ...
  const insertSuggestion = useCallback(
    (note: Note) => {
      ...
      if (wikilinkContext) {
        ...
        setTimeout(() => {
          const newCursorPos =
            wikilinkContext.start + `[[${note.title}]]`.length;
  ...
  useEffect(() => {
    ...
    const handleDocumentClick = (e: MouseEvent) => {
      ...
      if (
        ...
        target.closest("[data-wikilink-autocomplete]")
      ) {
        return;
      }
