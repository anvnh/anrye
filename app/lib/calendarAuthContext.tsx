...
export function CalendarAuthProvider({ children }: { children: ReactNode }) {
  const [isAuthenticated, setIsAuthenticated] = useState(true); // Skip connection check - set to true by default
  const [isLoading, setIsLoading] = useState(false); // Skip loading state
  const [userInfo, setUserInfo] = useState<{ name?: string; email?: string } | null>(null);

  // Skip authentication check - calendar will load events immediately
  // useEffect(() => {
